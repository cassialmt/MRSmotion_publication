function MRS_struct = GannetSegment5(MRS_struct)
...
for kk = 1:length(vox)
    ...
    for ii = 1:MRS_struct.p.numScans
        ...
        bone = [T1dir '/c4' T1name T1ext];
        soft= [T1dir '/c5' T1name T1ext]; %soft tissue
        air = [T1dir '/c6' T1name T1ext];
        
        GMvol  = spm_vol(GM);
        WMvol  = spm_vol(WM);
        CSFvol = spm_vol(CSF);
        softvol = spm_vol(soft);
        bonevol = spm_vol(bone);
        airvol = spm_vol(air);
        ...
        bonevol_tmp = bonevol.private.dat(:,:,:);
        bonevol_tmp(bonevol_tmp < 0.9) = NaN;
        bonevol_thresh = bonevol_tmp .* T1_tmp;
        bonevol_thresh = bonevol_thresh(:);
 
        softvol_tmp = softvol.private.dat(:,:,:);
        softvol_tmp(softvol_tmp < 0.9) = NaN;
        softvol_thresh = softvol_tmp .* T1_tmp;
        softvol_thresh = softvol_thresh(:);
        ...
        MRS_struct.out.QA.CNR(ii)   = abs(mean(WMvol_thresh, 'omitnan') - mean(GMvol_thresh, 'omitnan')) / ...
                                          sqrt(var(airvol_thresh, 'omitnan') + var(bonevol_thresh, 'omitnan')+ var(softvol_thresh, 'omitnan')  + var(WMvol_thresh, 'omitnan') + var(GMvol_thresh, 'omitnan'));
        ...
        % bone
        O_bonevox.fname = fullfile(a, [b '_bone' c]);
        O_bonevox.descrip = 'MRS_voxel_mask_bone';
        O_bonevox.dim = voxmaskvol.dim;
        O_bonevox.dt = voxmaskvol.dt;
        O_bonevox.mat = voxmaskvol.mat;
        bone_voxmask_vol = bonevol.private.dat(:,:,:) .* voxmaskvol.private.dat(:,:,:);
        O_bonevox = spm_write_vol(O_bonevox, bone_voxmask_vol);

        % soft
        O_softvox.fname = fullfile(a, [b '_soft' c]);
        O_softvox.descrip = 'MRS_voxel_mask_soft';
        O_softvox.dim = voxmaskvol.dim;
        O_softvox.dt = voxmaskvol.dt;
        O_softvox.mat = voxmaskvol.mat;
        soft_voxmask_vol = softvol.private.dat(:,:,:) .* voxmaskvol.private.dat(:,:,:);
        O_softvox = spm_write_vol(O_softvox, soft_voxmask_vol);
        ...
        bonesum = sum(sum(sum(O_bonevox.private.dat(:,:,:))));
        softsum = sum(sum(sum(O_softvox.private.dat(:,:,:))));

        fGM  = GMsum / (GMsum + WMsum + CSFsum + bonesum + softsum);
        fWM  = WMsum / (GMsum + WMsum + CSFsum + bonesum+ softsum);
        fCSF = CSFsum / (GMsum + WMsum + CSFsum + bonesum+ softsum);
        fbone = bonesum / (GMsum + WMsum + CSFsum + bonesum+ softsum);
        fsoft = softsum / (GMsum + WMsum + CSFsum + bonesum+ softsum);

        MRS_struct.out.(vox{kk}).tissue.fGM(ii)  = fGM;
        MRS_struct.out.(vox{kk}).tissue.fWM(ii)  = fWM;
        MRS_struct.out.(vox{kk}).tissue.fCSF(ii) = fCSF;
        MRS_struct.out.(vox{kk}).tissue.fbone(ii) = fbone;
        MRS_struct.out.(vox{kk}).tissue.fsoft(ii) = fsoft;
        ...
        tmp = sprintf(' %.2f', MRS_struct.out.(vox{kk}).tissue.fbone(ii));
        text(0.5, text_pos-0.48, 'bone voxel fraction: ', 'Units', 'normalized', 'FontName', 'Arial', 'HorizontalAlignment','right', 'VerticalAlignment', 'top', 'FontSize', 13);
        text(0.5, text_pos-0.48, tmp, 'Units', 'normalized', 'FontName', 'Arial', 'VerticalAlignment', 'top', 'FontSize', 13);
 
        tmp = sprintf(' %.2f', MRS_struct.out.(vox{kk}).tissue.fsoft(ii));
        text(0.5, text_pos-0.60, 'soft tissue voxel fraction: ', 'Units', 'normalized', 'FontName', 'Arial', 'HorizontalAlignment','right', 'VerticalAlignment', 'top', 'FontSize', 13);
        text(0.5, text_pos-0.60, tmp, 'Units', 'normalized', 'FontName', 'Arial', 'VerticalAlignment', 'top', 'FontSize', 13);
        ...
        MRS_struct.mask.(vox{kk}).img_montage{ii} = PlotSegmentedVoxels(struc, voxoff, voxmaskvol, O_GMvox, O_WMvox, O_CSFvox,O_bonevox,O_softvox);
...
function img_montage = PlotSegmentedVoxels(struc, voxoff, voxmaskvol, O_GMvox, O_WMvox, O_CSFvox,O_bonevox,O_softvox)
...
mask_t_bone = flipud(voxel2world_space(O_bonevox, voxoff));
mask_t_soft = flipud(voxel2world_space(O_softvox, voxoff));
...
mask_t_bone(mask_t_bone(:) < vox_mn) = vox_mn;
mask_t_bone(mask_t_bone(:) > vox_mx) = vox_mx;
mask_t_bone = (mask_t_bone - vox_mn) / (vox_mx - vox_mn);

mask_t_soft(mask_t_soft(:) < vox_mn) = vox_mn;
mask_t_soft(mask_t_soft(:) > vox_mx) = vox_mx;
mask_t_soft = (mask_t_soft - vox_mn) / (vox_mx - vox_mn);

mask_t     = 0.5 * mask_t;
mask_t_GM  = 0.8 * mask_t_GM;
mask_t_WM  = 0.8 * mask_t_WM;
mask_t_CSF = 0.8 * mask_t_CSF;
mask_t_bone = 0.8 * mask_t_bone;
mask_t_soft = 0.8 * mask_t_soft;
...
c_mask_t_bone = c_img_t + cat(3, mask_t_bone * vox_color(1,1), mask_t_bone * vox_color(1,2), mask_t_bone * vox_color(1,3));
c_mask_t_soft = c_img_t + cat(3, mask_t_soft * vox_color(1,1), mask_t_soft * vox_color(1,2), mask_t_soft * vox_color(1,3));

w_mask_t     = w_t + mask_t;
w_mask_t_GM  = w_t + mask_t_GM;
w_mask_t_WM  = w_t + mask_t_WM;
w_mask_t_CSF = w_t + mask_t_CSF;
w_mask_t_bone = w_t + mask_t_bone;
w_mask_t_soft = w_t + mask_t_soft;
...
img_t_bone = repmat(1 - w_mask_t_bone, [1 1 3]) .* img_t_bone + c_mask_t_bone;
img_t_soft = repmat(1 - w_mask_t_soft, [1 1 3]) .* img_t_soft + c_mask_t_soft;
...
img_t_bone(img_t_bone < 0) = 0; img_t_bone(img_t_bone > 1) = 1;
img_t_soft(img_t_soft < 0) = 0; img_t_soft(img_t_soft > 1) = 1;

img_montage = cat(2, img_t, img_t_GM, img_t_WM, img_t_CSF, img_t_bone, img_t_soft);
...
text(4*floor(size(mask_t,2)) + floor(size(mask_t,2)/2), 20, 'bone', 'Color', [1 1 1], 'FontSize', 20, 'HorizontalAlignment', 'center');
text(5*floor(size(mask_t,2)) + floor(size(mask_t,2)/2), 20, 'soft', 'Color', [1 1 1], 'FontSize', 20, 'HorizontalAlignment', 'center');
